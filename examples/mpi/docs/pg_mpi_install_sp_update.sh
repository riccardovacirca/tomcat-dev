...
set -e
# ==============================================================================
# ./pg_mpi_install_sp_update.sh -h <host> -u <user> -p <pass> -d <db_name>
# Installazione singola stored procedure UPDATE
# ==============================================================================
PG_CONTAINER=""
PG_USERNAME=""
PG_PASSWORD=""
...
SQL_FILE="pg_mpi_install_sp_update_v6.sql"

echo "========================================"
echo "PostgreSQL MPI - Install SP UPDATE"
echo "========================================"
...
echo "🔄 Installazione stored procedure UPDATE..."

# Esegui il file SQL
if docker exec -i "$PG_CONTAINER" psql -U "$PG_USERNAME" -d "$PG_DATABASE" < "$SCRIPT_DIR/$SQL_FILE"; then
    echo ""
    echo "✅ Stored procedure UPDATE installata con successo"
else
    echo ""
    echo "❌ Errore durante l'installazione della stored procedure UPDATE"
    exit 1
fi
...
procedure_count=$(docker exec -i "$PG_CONTAINER" psql -U "$PG_USERNAME" -d "$PG_DATABASE" -c "
    ...
    AND routine_name = 'sp_update_anagrafica_transazionale';
" -t 2>/dev/null | tr -d ' ')

if [[ "$procedure_count" -eq "1" ]]; then
    echo "✅ Stored procedure sp_update_anagrafica_transazionale verificata"
else
...
echo ""
echo "========================================"
echo "INSTALLAZIONE SP UPDATE COMPLETATA"
echo "========================================"