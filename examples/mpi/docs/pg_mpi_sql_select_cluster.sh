#!/bin/bash
set -e
# ==============================================================================
# ./pg_mpi_sql_select_cluster.sh -h <host> -u <user> -p <pass> -d <db_name>
# Script per visualizzazione cluster anagrafici PostgreSQL MPI
# ==============================================================================
PG_CONTAINER=""
PG_USERNAME=""
PG_PASSWORD=""
PG_DATABASE=""

while getopts "h:u:p:d:" opt; do
  case $opt in
    h) PG_CONTAINER="$OPTARG" ;;
    u) PG_USERNAME="$OPTARG" ;;
    p) PG_PASSWORD="$OPTARG" ;;
    d) PG_DATABASE="$OPTARG" ;;
    \?) echo "Opzione non valida: -$OPTARG" >&2; exit 1 ;;
  esac
done

# Verifica parametri obbligatori
if [[ -z "$PG_CONTAINER" || -z "$PG_USERNAME" || -z "$PG_PASSWORD" || -z "$PG_DATABASE" ]]; then
    echo "Uso: $0 -h <container> -u <user> -p <password> -d <database>"
    exit 1
fi

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SQL_FILE="pg_mpi_sql_select_cluster_v6.sql"

# Verifica esistenza container PostgreSQL
if ! docker ps --format "table {{.Names}}" | grep -q "^${PG_CONTAINER}$"; then
  echo "ERRORE: Container PostgreSQL '${PG_CONTAINER}' non trovato"
  exit 1
fi

# Verifica esistenza file SQL
if [[ ! -f "${SCRIPT_DIR}/${SQL_FILE}" ]]; then
  echo "ERRORE: File SQL non trovato: ${SQL_FILE}"
  exit 1
fi

# Header rimosso per output più pulito

# Esegui query cluster
docker exec -i "$PG_CONTAINER" env PGPASSWORD="$PG_PASSWORD" psql -U "$PG_USERNAME" \
  -d "$PG_DATABASE" < "${SCRIPT_DIR}/${SQL_FILE}"